...
class MCMC_MH:
    def __init__(self, likelihood, proposal_distribution, priors, num_samples, num_chains, stepsize=0.5, burnin_ratio=0.05, resume=False):
        ...
        elif os.path.exists('./checkpoints/TT/checkpoint_burnin.pkl') or os.path.exists('./checkpoints/TT/checkpoint_production.pkl'):
    ...
    def save_checkpoint(self, iteration, burn_in=False):
        if not os.path.exists('./checkpoints/TT'):
            os.makedirs('./checkpoints/TT')
        filename = f'./checkpoints/TT/checkpoint_{"burnin" if burn_in else "production"}.pkl'
        with open(filename, 'wb') as f:
            pickle.dump((self.curr_state, self.samples, self.likelihoods, self.R_minus_one, self.count), f)

    def load_checkpoint(self):
        checkpoint_file_prod = './checkpoints/TT/checkpoint_production.pkl'
        checkpoint_file_burnin = './checkpoints/TT/checkpoint_burnin.pkl'
...
if __name__ == "__main__":
    likelihood_function = PlanckLitePy(spectra='TT')
    priors = [
        norm(loc=0.0224, scale=np.sqrt(1)*0.0001),
        norm(loc=0.12, scale=np.sqrt(1)*0.001),
        norm(loc=1.04109, scale=np.sqrt(1)*0.0004),
        norm(loc=0.055, scale=np.sqrt(1)*0.006),
        norm(loc=0.965, scale=np.sqrt(1)*0.004),
        norm(loc=3.05, scale=np.sqrt(1)*0.001)
    ]
    mcmc = MCMC_MH(likelihood_function, proposal_distribution_3, priors, 20000, num_chains=4, stepsize=0.1, burnin_ratio=0.00, resume=False)
